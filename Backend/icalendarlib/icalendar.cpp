...
void ICalendar::LoadFromStream(std::istream &in) {
    ...
    // Prime NextLine by reading the first line outside the loop
    if (!std::getline(in, NextLine)) {
        // If the stream is empty, bail out
        return;
    }
    ...
    while (!in.fail()) {
        ...
        for (;;) {
            ...
            if (NextLine.empty() || (NextLine[0] != ' ' && NextLine[0] != '\t')) {
                // Not a folded line; break out so we can handle it in the next iteration
                break;
            }
            // If it's folded, unwrap it
...
void ICalendar::LoadFromString(const std::string &icalData)
{
    std::istringstream iss(icalData);
    LoadFromStream(iss);
}
